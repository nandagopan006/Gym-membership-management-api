...
class OwnerPaymentUpdateAPIView(APIView):
    ...
    @extend_schema(
        request=OwnerPaymentUpdateSerializer,
        responses={200: OwnerPaymentUpdateSerializer, 400: dict},
    )
    def patch(self, request, id):

        payment = get_object_or_404(Payment, id=id)

        serializer = OwnerPaymentUpdateSerializer(
